...
impl From<LC<components::Table>> for Table {
    fn from(mut lc: LC<components::Table>) -> Self {
...
impl Row {
    fn from_at_pos(position: i32, lc: LC<components::Row>) -> Self {
        let region = Region::from(lc.region);

        match lc.component {
            components::Row::Content { mut cells } => Self::from(ContentRow {
                region,
                position,
                cells: cells
                    .drain(..)
...
impl Cell {
    fn from_at_pos(
        row_position: i32,
        position: i32,
        lc: LC<components::Cell>,
    ) -> Self {
        let region = Region::from(lc.region);

        match lc.component {
            components::Cell::Content(mut x) => Self::from(ContentCell {
                region,
                row_position,
                position,
                contents: x
            ...
            components::Cell::SpanAbove => Self::from(SpanAboveCell {
                region,
                row_position,
                position,
            }),
            components::Cell::SpanLeft => Self::from(SpanLeftCell {
                region,
                row_position,
                position,
            }),
